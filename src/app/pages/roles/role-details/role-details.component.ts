...
export class RoleDetailsComponent {
  ...
  onToggle(
    ...
    const controller = this.rolesList.find(
      (c) => c.controller === controllerName
    );
    if (controller) {
  ...
  updateCheckedStatus(rolesFromDetails: any[], rolesList: any[]) {
    rolesList.forEach((controllerObj) => {
      const firstController = rolesFromDetails.find(
        (controller) => controller.controller === controllerObj.controller
      );
  ...
  setRolesToSubmit(arraySelected: any) {
    const transformed: any = Object.values(
      (arraySelected as any[]).reduce((acc: any, { controller, actions }) => {
        ...
        acc[controller].actions = [
          ...new Set([...acc[controller].actions, ...actions]),
        ];
        return acc;
      }, {})
    );
